...
# run download
Strat_data <- sapply(1:num_strat, download_func, simplify = TRUE)
...
sf_data <- do.call(rbind, Strat_data)

# Specify destination
fileDest <- "/Volumes/Pocillopora/Geodiversity Data/Raw/Macrostrat/"

# write to file
suppressWarnings(write_sf(sf_data, paste0(fileDest, "Macrostrat.shp")))
